...
const read = async (req, res) => {
    try {
        ...
        const memories = await Memory.find(filter).sort({ createdAt: -1 }).populate('people').populate('songId')
...
const readOne = async (req, res) => {
    try {
        const memory = await Memory.findOne({ _id: req.params.id, userId: req.user._id }).populate('people')
